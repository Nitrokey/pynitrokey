...
from pynitrokey.pro.gnuk_token import get_gnuk_device
from pynitrokey.pro.usb_strings import get_devices as get_devices_strings

from pynitrokey.pro.upgrade_by_passwd import validate_gnuk, validate_regnual, logger,\
    pro_update, DEFAULT_WAIT_FOR_REENUMERATION, DEFAULT_PW3, IS_LINUX, \
    show_kdf_details
...
@click.option(
    ...
    help="Use firmware for early 'Nitrokey Pro' key hardware revisions"
)
def update(regnual, gnuk, default_password, password, wait_e, keyno, verbose, yes,
    ...
    if green_led and (regnual is None or gnuk is None):
        local_critical(
            ...
            "use one from: https://github.com/Nitrokey/nitrokey-pro-firmware)")
...
pro.add_command(set_identity)
pro.add_command(update)
pro.add_command(kdf_details)
