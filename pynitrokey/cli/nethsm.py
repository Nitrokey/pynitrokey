...
from typing import Any, BinaryIO, Iterable, Iterator, Optional, Protocol, Sequence
...
def import_pkcs12(
    ...
    If the key IS is not set, it is generated by the NetHSM.

    This command requires authentication as a user with the Administrator
    role."""
    mechanisms = list(mechanisms)
    pkcs12_data = filename.read()

    try:
        private_key, certificate, additional_certificates = load_key_and_certificates(
            pkcs12_data, password if password is None else bytes(password, "utf-8")
        )
    except ValueError as e:
        raise CliException(f"Failed to parse PKCS#12 archive: {e}", support_hint=False)

    if private_key is None:
        raise click.ClickException("PKCS#12 archive does not contain a private key.")
    if certificate is None:
        raise click.ClickException(
            "PKCS#12 archive does not contain a certificate that matches the private key."
        )
    if len(additional_certificates) > 0:
        print("PKCS#12 archive contains additional certificates, which will be ignored.")

    accepted_key_types = (rsa.RSAPrivateKey, ec.EllipticCurvePrivateKey, ed25519.Ed25519PrivateKey)
    if not isinstance(private_key, accepted_key_types):
        raise click.ClickException(
            f"Unsupported private key type: {type(private_key).__name__}. Only RSA, EC, and Ed25519 are allowed."
        )
