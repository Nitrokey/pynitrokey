...
import pynitrokey.pro.rsa as rsa
from pynitrokey.pro.gnuk_token import get_gnuk_device, gnuk_devices_by_vidpid, \
    regnual, SHA256_OID_PREFIX, crc32, parse_kdf_data
from pynitrokey.pro.kdf_calc import kdf_calc
#from pynitrokey.pro.threaded_log import ThreadLog
from pynitrokey.pro.usb_strings import get_devices, print_device
from pynitrokey.pro.rsa_pub_key import rsa_key_data
...
def get_latest_release_data():
    try:
        # @todo: move to confconsts.py
        r = requests.get('https://api.github.com/repos/Nitrokey/nitrokey-pro-firmware/releases')
        json = r.json()
...
REMOTE_PATH = 'https://raw.githubusercontent.com/Nitrokey/nitrokey-pro-firmware/gnuk1.2-regnual-fix/prebuilt'
FIRMWARE_URL = {
...
def pro_update(regnual, gnuk, default_password, password, wait_e, keyno, verbose, yes,
        skip_bootloader, green_led):

    # @todo: move to some more generic position...
    local_print('Nitrokey Pro firmware update tool')
