...
class gnuk_token(object):
    def __init__(
        ...
        for intf in configuration.interfaces:
            for alt in intf:
                if (
                    alt.interfaceClass == HID_CLASS
                    and alt.interfaceSubClass == HID_SUBCLASS_NO_BOOT
                    and alt.interfaceProtocol == HID_PROTOCOL_0
                ):
    ...
    def mem_info(self):
        mem = self.__devhandle.controlMsg(
            requestType=0xC0, request=0, buffer=8, value=0, index=0, timeout=10
        )
        ...
        while count * 256 < data_len:
            if count == 0:
                ...
                if len(data[256 * count : 256 * count + 128]) < 128:
                    cmd_data0 = iso7816_compose(
                        ins, count, 0x00, data[256 * count : 256 * count + 128]
                    )
                    cmd_data1 = None
                else:
                    cmd_data0 = iso7816_compose(
                        ins, count, 0x00, data[256 * count : 256 * count + 128], 0x10
                    )
                    cmd_data1 = iso7816_compose(
                        ins, count, 0x00, data[256 * count + 128 : 256 * (count + 1)]
                    )
...
class regnual(object):
    ...
    def mem_info(self):
        mem = self.__devhandle.controlMsg(
            requestType=0xC0, request=0, buffer=8, value=0, index=0, timeout=10000
        )
        start = ((mem[3] * 256 + mem[2]) * 256 + mem[1]) * 256 + mem[0]
        end = ((mem[7] * 256 + mem[6]) * 256 + mem[5]) * 256 + mem[4]
        return (start, end)
    ...
    def download(self, start, data, verbose=False, progress_func=None):
        ...
        while addr < addr_end:
            ...
            res = self.__devhandle.controlMsg(
                requestType=0xC0, request=2, buffer=4, value=0, index=0, timeout=10000
            )
    ...
    def protect(self):
        self.__devhandle.controlMsg(
            requestType=0x40, request=4, buffer=None, value=0, index=0, timeout=10000
        )
        time.sleep(0.100)
        res = self.__devhandle.controlMsg(
            requestType=0xC0, request=2, buffer=4, value=0, index=0, timeout=10000
    ...
    def finish(self):
        self.__devhandle.controlMsg(
            requestType=0x40, request=5, buffer=None, value=0, index=0, timeout=10000
        )
...
def gnuk_devices_by_vidpid() -> Iterator[usb.Device]:
    try:
        ...
        print(
            "Warning: no backend was found to use for communication. "
