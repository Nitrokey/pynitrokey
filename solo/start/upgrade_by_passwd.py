...
def main(
    ...
    if reg is None and not skip_bootloader:
        ...
        local_print(
            "*** Running update. Do NOT remove the device from the USB slot, "
    ...
    if reg is None:
        ...
        # while reg == None:
        local_print(
            "  Wait {} second{}...".format(wait_e, "s" if wait_e > 1 else ""), end=""
        )
...
def start_update(
    ...
    local_print(
        ...
        "{} (published: {}),\n provided firmware: {}".format(
            latest_tag["tag_name"], latest_tag["published_at"], gnuk
        )
    )
    ...
    if not update_done:
        local_print()
        local_print(
            "*** Could not proceed with the update. Please execute one or all of the following and try again:\n"
